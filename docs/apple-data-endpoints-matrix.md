...
| `GET https://apps.apple.com/us/iphone/search?term={keyword}` | Primary keyword order + initial app docs | `serialized-server-data` lockups: `adamId`, `title`, `subtitle`, `developerName`, `rating`, `ratingCount`, `icon`; plus `nextPage.results[].id` (`type=apps`) | lockup fields seed `aso_apps` docs and keyword order/app count (`developerName -> publisherName`) | Primary order source; a missing `data[0].data.nextPage` is treated as an incomplete response and falls back to MZSearch order. Other parse/contract failures also fall back. `rating`/`ratingCount` here are bootstrap values and can be rounded/approximate. Dashboard add-app search uses lockup `id/name/icon` directly and does not run lookup hydration in this path. |
...
2. If the search page fails parsing or omits `data[0].data.nextPage`, use MZSearch for order.
