...
2. Use MZSearch only for unresolved fields: count when `nextPage` is absent; order and count when primary order is unavailable. Clamp a non-empty fallback count to at least the known primary-order length; treat an empty fallback as unresolved when primary contains apps.
