...
- When `nextPage` is absent, the usable primary search-shelf order remains authoritative for its returned positions. MZSearch extends that prefix with unseen IDs and supplies the complete count; it cannot reorder or replace primary positions. When the leading primary search shelf is missing or malformed, MZSearch supplies the full order. Tail-only primary IDs are not considered a usable primary order.
- App order includes both organic app-bearing row types: `AppSearchResult/content` and `AppEventSearchResult/appEvent`. An app-event row represents that app's search slot, not a second result or a paid-ad signal, and app IDs are deduplicated so the app is counted once. Explicit ad rows, editorial, bundle, and unknown row types are ignored and do not invalidate usable app order.
...
- When MZSearch supplies a non-empty result, it can extend a usable primary order only when it is at least as long as that prefix; otherwise the primary order is retained and `appCount` is clamped to its length. If no usable primary order exists, MZSearch supplies the full order. An empty MZSearch fallback is contradictory and remains unresolved when the primary response contains apps; it resolves to zero only when the primary response also contains no apps. A single unresolved field remains nullable during order refresh so persistence can retain its last complete stored value instead of writing zero; when both count and order are unresolved, persistence is skipped.
- App detail sources:
