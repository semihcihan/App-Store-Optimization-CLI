...
- Search-shelf parsing and drift validation are limited to the app-bearing row types the runtime consumes: `AppSearchResult/content` and `AppEventSearchResult/appEvent`. Editorial, bundle, and unknown row types are ignored and do not invalidate usable app order.
