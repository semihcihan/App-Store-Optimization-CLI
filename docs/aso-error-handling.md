...
- Transport failures (`429`, `5xx`, timeout, and network failures) never enter the contract-drift reporter. Recovered source failures remain silent; when required enrichment fields remain unresolved, the final source error is retained through persisted keyword-failure metadata. If every requested keyword ends in a retryable upstream failure, the terminal aggregate is classified separately as an upstream failure. Order-only refreshes can still return usable partial data without advancing freshness when both order fields are unresolved.
