...
describe("keyword-pipeline-service", () => {
  ...
  it("skips enrichment when keyword popularity is below min threshold", async () => {
    ...
    mockEnrichAsoKeywordsLocal.mockImplementation(async (_country, items) => {
      ...
      throw new Error(`Unexpected enrich keyword: ${keyword}`);
    });

    const result = await keywordPipelineService.run("US", ["low", "high"], {
      filters: { minPopularity: 10 },
    });

    expect(result.items.map((item) => item.keyword)).toEqual(["high"]);
    ...
    expect(mockEnrichAsoKeywordsLocal).toHaveBeenCalledTimes(1);
    expect(mockEnrichAsoKeywordsLocal).toHaveBeenCalledWith("US", [
      { keyword: "high", popularity: 40 },
    ]);
    expect(getKeyword("US", "low")?.popularity).toBe(8);
    expect(getKeyword("US", "low")?.difficultyScore).toBeNull();
  });

  it("filters out high-difficulty keywords after enrichment", async () => {
    mockLookupAsoCacheLocal.mockResolvedValue({
      hits: [],
      misses: ["easy", "hard"],
    });
    ...
    const result = await keywordPipelineService.run("US", ["easy", "hard"], {
      filters: { maxDifficulty: 30 },
    });

    expect(result.items.map((item) => item.keyword)).toEqual(["easy"]);
    expect(result.failedKeywords).toEqual([]);
    expect(result.filteredOut).toEqual([
      {
        keyword: "hard",
