...
const DEFAULT_ASO_COUNTRY = "US";
const APP_STORE_LANGUAGE_BY_COUNTRY: Record<string, string> = {
  US: "en-us",
};
const APP_STORE_ICON_IMAGE_URL =
...
function getDashboardApiErrorCode(error: unknown): string | null {
  if (!(error instanceof DashboardApiError)) return null;
  return typeof error.errorCode === "string" ? error.errorCode : null;
}

function isAuthFlowErrorCode(code: string | null): boolean {
  return (
    code === "AUTH_REQUIRED" ||
...
function authFlowErrorMessage(code: string | null): string {
...
function toActionableErrorMessage(error: unknown, fallbackMessage: string): string {
...
async function apiWrite<T>(method: "POST" | "DELETE", path: string, body: unknown): Promise<T> {
...
const normalizeIconUrl = (rawUrl: string): string =>
  String(rawUrl)
    .replace(/\{w\}/g, "100")
    .replace(/\{h\}/g, "100")
    .replace(/\{c\}/g, "")
...
const getIconUrl = (doc: AppDoc | undefined): string | null => {
  if (!doc) return null;
  ...
  const iconArtworkUrl = typeof doc.iconArtwork?.url === "string" ? doc.iconArtwork.url : null;
  ...
  const candidates = [iconTemplate, iconUrl, iconArtworkUrl, doc.artworkUrl512, doc.artworkUrl100].filter(
...
const getAppStoreLanguageForCountry = (country: string): string => {
...
const buildAppStoreUrl = (appId: string, country: string): string => {
...
const getChange = (row: Row): number | null => {
...
const formatRatingValue = (value: number, locale?: string): string =>
  new Intl.NumberFormat(locale, {
    minimumFractionDigits: 1,
    maximumFractionDigits: 1,
  }).format(value);

const getNumberDelta = (
...
const buildTopAppRows = (data: KeywordDetails): TopAppRow[] =>
